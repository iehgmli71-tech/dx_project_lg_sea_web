...
Future<void> showPrayerScheduleSheet(
    BuildContext context, {
      ...
      /// 🔥 각 가전의 현재 전원 상태 (시트 바깥에서 관리하던 값)
      required bool isWasherOn,
      required bool isFridgeOn,
...
class _PrayerScheduleSheet extends StatefulWidget {
  ...
  /// 🔥 시트가 열릴 때의 가전 전원 상태
  final bool isWasherOn;
  final bool isFridgeOn;
  final bool isAcOn;
  ...
  const _PrayerScheduleSheet({
    ...
    required this.isWasherOn,
    required this.isFridgeOn,
...
class _PrayerScheduleSheetState extends State<_PrayerScheduleSheet> {
  ...
  List<_MonthData> _generateCalendar() {
    ...
    while (!current.isAfter(lastDate)) {
      ...
      // Dart weekday: Mon=1 ... Sun=7 → Sun=0 으로 맞추기
  ...
  /// 달력에서 “설정 완료” 눌렀을 때
  /// → 상태만 켜고 ESP 전송은 하지 않음 (메인 [적용] 버튼에서 처리)
  void _handleCalendarConfirm() {
    if (startDate != null && endDate != null) {
      setState(() {
        ...
        washerDryerDelay = true;
        fridgeDoorAlert = true;
        showCalendar = false; // 달력 오버레이만 숨김
        selectingStartDate = true;
      });

      // CardMode 상단 배너 등에 반영
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            style: ElevatedButton.styleFrom(
                              elevation: (washerDryerDelay ||
                                  fridgeDoorAlert ||
                                  ramadanEcoMode)
                                  ? 6
                                  : 0,
                              backgroundColor: (washerDryerDelay ||
                                  fridgeDoorAlert ||
                                  ramadanEcoMode)
                                  ? const Color(0xFF22C55E)
                                  : const Color(0xFFE5E7EB),
                              foregroundColor: (washerDryerDelay ||
                                  fridgeDoorAlert ||
                                  ramadanEcoMode)
                                  ? Colors.white
                            ...
                            (washerDryerDelay || fridgeDoorAlert || ramadanEcoMode)
                                ? () async {
                              // ✅ Quiet Home Mode: 세 ESP32 모두
                              if (washerDryerDelay || fridgeDoorAlert) {
                                await Esp32Api.showQuietHomeModeAll();
                              }

                              // ✅ Ramadan Mode: 세 ESP32 모두
                              if (ramadanEcoMode) {
