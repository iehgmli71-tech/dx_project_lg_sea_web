...
class _CardModeState extends State<CardMode> {
  ...
  void initState() {
    super.initState();
    // ✅ LG Regen(CardMode)에 들어온 순간:
    //    세 ESP32 모두에 "Hello Regen" + LED OFF 보내기
    Esp32Api.showHelloRegenAll();
  }

  // ✅ 특정 id의 디바이스가 현재 켜져 있는지 확인하는 헬퍼
  bool _isDeviceOn(String deviceId) {
    try {
      final d = devices.firstWhere((device) => device.id == deviceId);
      return d.active;
    } catch (_) {
      // 해당 id 디바이스가 없으면 꺼져 있다고 취급
      return false;
    }
  }
  ...
  void _handleDeviceStatusChange(String deviceId, bool isOn) {
    ...
    // 2) 하드웨어 제어 (비동기, 기다릴 필요 X)
    if (isOn) {
      if (deviceId == '1') {
        // 세탁기 전원 ON → "Wash Contact" + 흰 LED
        Esp32Api.washerPowerOn();
      } else if (deviceId == '3') {
        // 냉장고 전원 ON → "Fridge Contact" + 흰 LED
        Esp32Api.fridgePowerOn();
      } else if (deviceId == '4') {
        // 에어컨 전원 ON → "AC Contact" + 흰 LED
        Esp32Api.acPowerOn();
      }
    }
  }
          ...
          children: [
            Row(
              ...
              children: [
                ...
                TextButton(
                  onPressed: () {
                    // ✅ Prayer Schedule 시트를 열 때
                    //    각 가전의 현재 전원 상태를 함께 넘겨준다.
                    showPrayerScheduleSheet(
                      ...
                      isWasherOn: _isDeviceOn('1'), // WashingMachine
                      isFridgeOn: _isDeviceOn('3'), // Refrigerator
                      isAcOn: _isDeviceOn('4'), // Air Conditioner
                    );
                  },
